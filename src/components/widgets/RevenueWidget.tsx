...
export default function RevenueWidget({ merchantId, storeId, startPeriod, endPeriod, selectedPeriod }) {
  ...
  return (
    <Card sx={{ display: 'flex', alignItems: 'center', p: 3 }}>
      <Box sx={{ flexGrow: 1 }}>
        <Typography variant="subtitle2" paragraph>Omsætning</Typography>
        <Typography variant="h3" gutterBottom>
          {data?.total ? fCurrency(Math.round(data?.total)) : '0 DKK'}
        </Typography>
        <TrendingInfo percent={data?.total_delta_percentage ?? 0} />
      </Box>
      <Chart
        type="line"
