...
import { GET_ACQUIRING_ERROR_RATE_WIDGET_DATA } from 'src/queries/widget';
...
export default function AcquiringErrorRateWidget({ merchantId, storeId, startPeriod, endPeriod, selectedPeriod }) {
  ...
  const { data, loading } = useQuery(GET_ACQUIRING_ERROR_RATE_WIDGET_DATA, {
    variables: {
      merchantId,
      storeId,
      startPeriod,
  ...
  return (
    <Card sx={{ display: 'flex', alignItems: 'center', p: 3 }}>
      <Box sx={{ flexGrow: 1 }}>
        <Typography variant="subtitle2" paragraph>Indløsningsfejlrate</Typography>
        <Typography variant="h3" gutterBottom>
          {data?.total_percentage ? fPercent(data?.total_percentage, '.00') : '0%'}
        </Typography>
        <TrendingInfo percent={data?.total_delta_percentage ? data?.total_delta_percentage : 0} inverse />
      </Box>
      <Chart
        type="line"
