...
import { GET_REPEAT_CUSTOMER_NUMBERS_WIDGET_DATA } from 'src/queries/widget';

// ----------------------------------------------------------------------

export default function RepeatCustomerNumbersWidget({
  merchantId,
  storeId,
  startPeriod,
  endPeriod,
  ...
  const { data, loading } = useQuery(GET_REPEAT_CUSTOMER_NUMBERS_WIDGET_DATA, {
    variables: {
      merchantId,
      storeId,
      startPeriod,
  ...
  return (
    <Card sx={{ display: 'flex', alignItems: 'center', p: 3 }}>
      <Box sx={{ flexGrow: 1 }}>
        <Typography variant="subtitle2" paragraph>Tilbagevendende kunder</Typography>
        <Typography variant="h3" gutterBottom>{data?.total ? fNumber(data?.total) : 0}</Typography>
        <TrendingInfo percent={data?.delta_percentage ?? 0} />
      </Box>
