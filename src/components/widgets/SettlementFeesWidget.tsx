  ...
  return (
    <Card sx={{ display: 'flex', alignItems: 'center', p: 3, ...sx }} {...other}>
      ...
      <Avatar sx={(theme) => ({ width: 120, height: 120, backgroundColor: theme.palette.background.neutral })}>
        <InsertDriveFileIcon sx={(theme) => ({ backgroundColor: theme.palette.grey[300], width: 24, height: 24, outline: `12px solid ${theme.palette.grey[300]}`, borderRadius: '50%' })}/>
      </Avatar>
    </Card>
  );
}

export default function SettlementFeesWidget({ merchantId, storeId, startPeriod, endPeriod, selectedPeriod }) {
  ...
  return (
    <Grid item xs={12} md={12}>
      <CardCost title="Afregningsgebyr" total={data?.total_percentage ?? 0} sx={{ '& .MuiAvatar-img': { objectFit: 'contain' } }} />
    </Grid>
  );
}
