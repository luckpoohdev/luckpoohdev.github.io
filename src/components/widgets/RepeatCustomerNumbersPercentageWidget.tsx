...
import { GET_REPEAT_CUSTOMER_NUMBERS_PERCENTAGE_WIDGET_DATA } from 'src/queries/widget';
...
export default function RepeatCustomerNumbersPercentageWidget({ merchantId, storeId, startPeriod, endPeriod, selectedPeriod }) {
  ...
  const { data, loading } = useQuery(GET_REPEAT_CUSTOMER_NUMBERS_PERCENTAGE_WIDGET_DATA, {
    variables: {
      merchantId,
      storeId,
      startPeriod,
...
console.log('repeat customer numbers percentage:', data);
  ...
  return (
    <Card sx={{ display: 'flex', alignItems: 'center', p: 3 }}>
      <Box sx={{ flexGrow: 1 }}>
        <Typography variant="subtitle2" paragraph>Tilbagevendende kunderate</Typography>
        <TrendingInfo percent={data?.total_delta_percentage ?? 0} inverse />
        <Typography variant="h3">
          {data?.total_percentage ? fPercent(data?.total_percentage, '.00') : '0%'}
        </Typography>
      </Box>
      <Chart
        type="line"
