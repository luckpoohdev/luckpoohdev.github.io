...
import { _teamCards } from 'src/_mock/arrays';
...
import {
  ...
  // sections
  import { TeamTableToolbar, TeamTableRow } from 'src/sections/@dashboard/teams/list';
...
export default function GeneralAppPage() {
  ...
  const [tableData, setTableData] = useState(teamList);
  ...
  const handleEditRow = (id) => {
    push(PATH_DASHBOARD.team.edit(paramCase(id)));
  };
  ...
  const handleCreateNewTeam = () => {
    router.push(PATH_APP.organization.teams.create())
  }
      ...
      <Container maxWidth={/*themeStretch ? false : 'xl'*/false} sx={{ pt: 1.5 }}>
        ...
        <Grid item container xs={12}>
          <Grid item xs={12} md={4}>
            ...
            Se og administrer teams forneden
            </Typography>
          </Grid>
          <Grid item container xs={12} md={8} display="flex" justifyContent="flex-end" alignItems="center" spacing={1}>
            <CustomButton onClick={handleCreateNewTeam} icon={<Iconify icon="material-symbols:add-rounded" />}>Opret nyt team</CustomButton>
          </Grid>
        </Grid>
...
function applyFilter({ inputData, comparator, filterKeywords, filterStatus }) {
  ...
  if (filterKeywords) {
    inputData = inputData.filter(
      (team) => (
        team.name.toLowerCase().indexOf(filterKeywords.toLowerCase()) !== -1
      )
    );
  }
  
  if (filterStatus !== 'all') {
    inputData = inputData.filter((team) => team.status === filterStatus);
  }
