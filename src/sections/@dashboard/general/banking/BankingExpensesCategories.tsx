...
export default function BankingExpensesCategories({ title, subheader, totalCountLabel, totalLabel, chart, ...other }: Props) {
  ...
  let total = 0
  const chartSeries = series.map((i) => {
    total += i.value
    return i.value
  });
    ...
    <StyledRoot {...other}>
      ...
      <Stack direction="row" divider={<Divider orientation="vertical" flexItem />}>
        <Box sx={{ py: 2, width: 1, textAlign: 'center' }}>
          ...
          <Typography sx={{ typography: 'h4' }}>{series.length}</Typography>
        </Box>
        ...
        <Box sx={{ py: 2, width: 1, textAlign: 'center' }}>
          ...
          <Typography sx={{ typography: 'h4' }}>{fCurrency(total)}</Typography>
        </Box>
      </Stack>
    </StyledRoot>
  );
